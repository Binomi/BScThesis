...
\begin{abstract}
In dieser Arbeit werden Spektren eines 80\,MHz-Ti:Sa-Oszillators in Echtzeit gemessen.
...
Hiermit gelingt es uns, Multipuls-Dynamiken zu enthüllen, die sich bei bestimmten Justagen des Laserresonators einstellen.
...
Abstand und relative Phase oszillieren, die relative Phase wächst stufenweise an, die Pulse kollidieren kontinuierlich, wir beobachten aber auch Pulse mit unterschiedlicher Geschwindigkeit, die annähernd unabhängig durch den Resonator propagieren.
Darüber hinaus werden diskrete Werte für stabile Pulsabstände identifiziert.
Die Messmethode bietet auch die Möglichkeit zur Beobachtung von transienten Vorgängen wie der Entstehung von Femtosekundenpuls-Paaren.
Mit Simulationen können verschiedene experimentelle Beobachtungen nachvollzogen werden.
%% Optional: Stichwoerter. Wenn nicht gewuenscht, koennen die beiden
%% folgenden Zeilen geloescht werden
  \bigskip\par
  \textbf{Stichwörter:} Physik, Bachelorarbeit, Echtzeitspektoskopie, Optische Solitonen, Multipulse
\end{abstract}
%% So laesst sich in die andere Sprache umschalten (Englisch bzw. Deutsch)
\begin{otherlanguage}{english}
\begin{abstract}
  In this paper the spectra of an 80\,MHz Ti:sapph oscillator are measured in real-time.
  For that purpose, a dispersive fiber is used which maps spectral information to temporal information.
  The latter can be recorded with high-speed electronics.
  Hereby we can reveal multipulse dynamics which arise at certain adjustments of the laser cavity.
  Especially double pulses are spotlighted.
  Their spectrum contains information about the pulse spacing and relative phase.
  Various types of dynamics are observed:
  spacing and relative phase are oscillating, the relative phase increases stepwise, periodically colliding pulses and pulses with different velocities propagating almost independently through the cavity.
  Beyond, we notice discrete values for stable pulse spacings.
  The used measurement technique allows also observations of transient processes like the build-up of a femtosecond pulse pair.
  Some of the experimental observations are related to simulations.
  \bigskip\par
  \textbf{Keywords:} Physics, Bachelor thesis, real-time spectroscopy, optical solitons, multipulses
\end{abstract}
\end{otherlanguage}
...
\begin{figure}[!htb]
   \centering
   \subfloat[Soft aperture KLM: Großer Überlapp zwischen gepulstem Strahl und Pumpstrahl.\label{fig:softApKLM}]
   {\includegraphics[width=0.49\textwidth]{figures/softApKLM.png}}
   \hfill
   \subfloat[Hard aperture KLM: Gepulster Strahl passiert Apertur ungehindert. \label{fig:hardApKLM}]
...
%\subsubsection{Titan:Saphir-Laser}
...
\begin{figure}[!htb]
   ...
   \vspace*{-3mm}   
   \caption{Gebundene Pulse: Zeitaufgelöste Spektren in Abhängigkeit der Pumpleistung. Relative Phase und Pulsabstand oszillieren.}
...
\begin{figure}[!htb]
   ...
   \caption{Kollisionszyklus bei einer Pumpleistung von $P_\text{Pump}=4.56\,$W\,.}
