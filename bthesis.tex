...
Im hier untersuchten System wird das durch den \textbf{Kerr-Effekt} erreicht, also den intensitätsabhängigen Teil des Brechungsindizes
\begin{align}
	n(I)=n_0+n_2\cdot I\,.
	\label{eq:kerr}
\end{align}
Für typische Lasermedien wie Titan:Saphir ist $n_2\sim$.
Für XXX muss der Puls intensiver als XXX sein.
...
\section{Simulation}
\label{sec:SimTheo}
Die Effekte der oben beschriebenen optischen Elemente auf das elektrische Feld im Laseresonator werden im Rahmen der Mastergleichung \cite{Haus} mathematisch beschrieben.
...
Davon ausgehend, dass das Pulspaar viel kürzer ist als die Resonatorlänge $T_\text{cav}$ und sich die Verstärkung $\alpha$ während der Pulse vernachlässigbar ändert, kann die obige Gleichung mit der Umlaufzeit $T_\text{cav}$ multipliziert und anschließend integriert werden:
\begin{align}
...
In Abbildung \ref{fig:PDBandbreite} XXX
In Abbildung \ref{fig:PDBandbreiteAusw} XXX
\begin{figure}[!htb]
   \centering
...
Modelockt der Laser, repräsentiert jeder XXX des dispergierten Signals ein Einzelschuss-Spektrum (vgl. Abb.\ref{fig:plainData})
Also wird zunächst die Pulswiederholrate bestimmt.
...
Die Gespräche mit ihm waren sehr konstruktiv.

Dr. Georg Herink für die gute Zusammenarbeit.
Außerdem der Besuch des Labors von Hauke Bensch vom Institut für Quantenoptik der Universität Hannover.
